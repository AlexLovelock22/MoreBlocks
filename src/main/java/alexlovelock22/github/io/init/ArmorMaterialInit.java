...
public class ArmorMaterialInit {
    ...
    public static RegistryEntry<ArmorMaterial> register(String id, Map<ArmorItem.Type, Integer> defensePoints,
                                                        ...
                                                        float knockbackResistenace, boolean dyeable){

        List<ArmorMaterial.Layer> layers = List.of(
                new ArmorMaterial.Layer(MoreBlocks.id(id), "", dyeable)
        );

        var material = new ArmorMaterial(defensePoints, enchantability, equipSound, repairIngredient, layers, toughness, knockbackResistenace);

        material = Registry.register(Registries.ARMOR_MATERIAL, MoreBlocks.id(id), material);
